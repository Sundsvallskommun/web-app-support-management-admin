...
export const DialogManageLabel: React.FC<ManageLabelProps> = ({
  ...
  const switchToLabel = (fullName: string) => {
    try {
      ...
      if (matchingChildIdx != -1) {
        // List where matching item is present has been found
  ...
  const findLabel = (fullName: string, nodes: LabelInterface[]): LabelInterface => {
    for (const index in nodes) {
      const node = nodes[index];
      const nodeFullName =
        (node.resourcePath?.length || 0) === 0 ?
          node.resourceName
        : node.resourcePath + PATH_SEPARATOR + node.resourceName;
  ...
  const washInput = (input: string): string => {
    const validatedInput = input.replace(/[^A-Z0-9_]/g, '');
  ...
  const handleNameInputChange = (input: string) => {
    ...
    future.labels?.forEach((m) => {
      m.resourcePath = future.resourceName;
    });
  ...
  const handleAddChild = () => {
    const future = {
      ...
      labels: [
        ...
        {
          resourcePath: labelProspect.resourcePath,
          resourceName: '',
          name: '',
          displayName: '',
  ...
  const getPossibleDuplicateNames = (structure: LabelInterface[]): string[] => {
    if (structure) {
      const names = flatten(structure).map((entry) =>
        entry.resourcePath ? entry.resourcePath + PATH_SEPARATOR + entry.resourceName : entry.resourceName
      );

      return Array.from(
        new Set(names.filter((name, index) => names.some((match, idx) => match === name && idx !== index)))
      );
    }
    return [];
  };
  ...
  useEffect(() => {
    ...
    setLabelProspect({
      resourcePath: existingLabel?.resourcePath || '',
  ...
  return (
    <Dialog
      ...
      label={
        existingLabel ?
          `${t('common:dialogs.manage_label.header_prefix_modify')} ${existingLabel?.displayName} ${t('common:in')} ${municipality?.name}`
    ...
    >
      <Dialog.Content>
        ...
        >
          <Dialog.Content>
            <div className="bottom-margin-50">
              ...
              <List className="bottom-margin-100" listStyle={'numbered'}>
              ...
              <div>{t('common:dialogs.manage_label.duplicates_found_suffix')}</div>
            </div>
          </Dialog.Content>
        ...
        <div className="d-flex bottom-margin-50">
          ...
          <div>
            {(labelProspect?.resourcePath || '').length > 0 && (
              <span>
                {(labelProspect?.resourceName || '').length > 0 && validChilds() && (
                  <Link onClick={() => switchToLabel(`${labelProspect?.resourcePath}`)}>
                    <LucideIcon
                      className="right-margin-50 label-up-navigation-icon hand-icon"
                      size={18}
                      name={'folder-up'}
                    />
                  </Link>
            ...
            {existingLabel ?
              <button disabled className="sk-chip chip-input">
                <span>{labelProspect?.resourcePath}</span>
              </button>
            : <Input.Group className="sk-inline-flex">
                <Input.LeftAddin>{labelProspect?.resourcePath}</Input.LeftAddin>
                <Input
                  size={'md'}
                  maxLength={250}
                ...
                <Table.Row key={'child_name_input' + m.id}>
                  <Table.Column>
                    <Link
                      disabled={
                        !(
                          labelProspect?.resourceName?.length > 0 &&
                          labelProspect?.classification?.length > 0 &&
                          labelProspect?.displayName?.length > 0 &&
                          validChilds()
                        )
                      }
                      onClick={() => switchToLabel(`${m?.resourcePath}.${m?.resourceName}`)}
                    >
                    ...
                    : <Input.Group className={'max-width'}>
                        <Input.LeftAddin>{m?.resourcePath}/</Input.LeftAddin>
                        <Input
                          className={'smaller'}
                          size={'md'}
                          placeholder={t('common:dialogs.manage_label.name_placeholder')}
